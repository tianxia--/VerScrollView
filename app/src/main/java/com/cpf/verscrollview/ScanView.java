...
public class ScanView extends RelativeLayout
{
	...
	private float lastY;
	// 前一页，当前页，下一页的左边位置
	private int prePageTop = 0, currPageTop = 0, nextPageTop = 0;
	// 三张页面
	...
	private void moveLeft(int which)
	{
		...
		{
			case PRE:
				prePageTop -= MOVE_SPEED;
				if (prePageTop < -mHeight)
					prePageTop = -mHeight;
				right = mHeight + prePageTop;
				break;
			case CURR:
				currPageTop -= MOVE_SPEED;
				if (currPageTop < -mHeight)
					currPageTop = -mHeight;
				right = mHeight + currPageTop;
				break;
		}
	}
	...
	Handler updateHandler = new Handler()
	{
		...
		public void handleMessage(Message msg)
		{
			...
			Log.e(TAG,"state = " + state + ",prePageTop = " + prePageTop + ",currPageTop = " + currPageTop + ",speed = " + speed + ",index = "+index);
			if (state != STATE_MOVE)
				return;
			// 移动页面
			// 翻回，先判断当前哪一页处于未返回状态
			if (prePageTop > -mHeight && speed < 0)
			{
				// 前一页处于未返回状态
				moveLeft(PRE);
				Log.e(TAG,"翻页 =1" );

			} else if (prePageTop == -mHeight &&currPageTop == 0 && speed >= 0)
			{
				Log.e(TAG,"翻页=2" );
				// 当前页处于未返回状态
				moveRight(CURR);
			} else if ((speed < 0 || currPageTop < 0) && index < adapter.getCount())
			{
				Log.e(TAG,"翻页=3" );
			...
			} else if ((speed >= 0 && prePageTop > -mHeight) && index > 1)
			{
				Log.e(TAG,"翻页=4" );
			...
			switch (event.getActionMasked())
			{
				...
				case MotionEvent.ACTION_MOVE:
							...
							&& mEvents == 0)
					{
						...
						{
							// 非第一页
							prePageTop += (int) moveLenght;
							// 防止滑过边界
							if (prePageTop > 0)
								prePageTop = 0;
							else if (prePageTop < -mHeight)
							{
							...
							&& mEvents == 0)
					{
						...
						{
							currPageTop += (int) moveLenght;
							// 防止滑过边界
							if (currPageTop < -mHeight)
								currPageTop = -mHeight;
							else if (currPageTop > 0)
							{
	...
	protected void dispatchDraw(Canvas canvas)
	{
		...
		if (right == 0 || right == mHeight)
			return;
		RectF rectF = new RectF(0, right, mWidth, mHeight);
		Paint paint = new Paint();
		paint.setAntiAlias(true);
		LinearGradient linearGradient = new LinearGradient(0, right,
				0, right + 36, 0xffbbbbbb, 0x00bbbbbb, TileMode.CLAMP);
	...
	protected void onLayout(boolean changed, int l, int t, int r, int b)
	{
		if (adapter == null)
			return;
		prePage.layout(0, prePageTop,
				prePage.getMeasuredWidth(),
				prePage.getMeasuredHeight() + prePageTop);
		currPage.layout(0, currPageTop,
				currPage.getMeasuredWidth(),
				currPage.getMeasuredHeight() + currPageTop);
		nextPage.layout(0, nextPageTop,
				nextPage.getMeasuredWidth(),
				nextPage.getMeasuredHeight() + nextPageTop);
		invalidate();
	}
